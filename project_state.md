...
## Meshtastic interop plan (`mesh` branch, planning only — nothing landed yet)
...
| 1 — PHY parity | Add `setPreambleLength()`, `enableCrc()`, and an explicit sync-word setter to the `hvLoRa` adapter interface (missing today on both the SX127x and RadioLib/SX126x sides); add the **full** Meshtastic region+preset tables (all 27 regions, not just the ones in range of the test hardware — region is a user-set field, see below). Bench-test the pair against whichever legal region the tester picks from the 7 in range of 433–510MHz. | A fixed test frame from this firmware shows up (even as "bad packet") in a real node's debug log at the right frequency/timing. |
...
7. Start Phase 0 of the Meshtastic interop plan above, on the `mesh` branch: pin a `meshtastic/firmware` commit and write up the exact `PacketHeader`/hash/encryption spec before any code changes.
