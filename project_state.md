...
8. ~~Confirm MeshCore mode on the real Heltec V3 (identity/channel join via Serial log)~~ **Done** — see "Verification" above. Still needed: an actual over-the-air channel message with a second MeshCore node (even a phone running the official app on the same channel name/PSK would do).
