...
- **Hardware**: confirmed on a real Heltec V3 in **both** modes after this branch's changes. Native mode: unchanged boot behavior on the MeshCore-enabled firmware (radio init, WiFi AP, web UI). MeshCore mode: `NETWORK.mode` set to `"meshcore"` and reboot correctly routes into the MeshCore backend — Serial log shows `radio_init()` succeeding against the real SX1262, `HasMeshCoreChat::begin()` completing (identity created/loaded, channel joined) with no error, and `HasMeshCoreLoop` running on the same core/task slot `HasTRX` uses in native mode. Clean boot, no crashes, in both modes.
- **Not yet tried at all:** the OLED showing anything in MeshCore mode (native's `initOLED()`/`logo()` calls are unconditional in `setup()`, unrelated to `networkMode`, so this should already work, just unconfirmed).

### Two real bugs found getting an actual over-the-air message working (both fixed)

Getting a second Heltec board (V2) added and a real over-the-air MeshCore channel message exchanged between it and the V3 surfaced two more real, silent-failure bugs, neither caught by build or single-node hardware verification:

1. **`MAX_GROUP_CHANNELS` was never defined as a build flag.** `BaseChatMesh::addChannel()` compiles to two entirely different bodies depending on whether that macro is defined (`helpers/BaseChatMesh.cpp`'s `#ifdef MAX_GROUP_CHANNELS` / `#else` split) — undefined, it's a stub that unconditionally `return NULL; // not supported`. `HasMeshCoreChat::begin()` was calling `addChannel()` without ever checking or logging the result, so channel-join silently failed on every boot, `sendChannelText()` silently returned `false` forever after, and neither board ever transmitted or received anything — no crash, no error, just total silence, indistinguishable from a WiFi/browser connectivity problem (which is what it looked like at first). Fixed with `-D MAX_GROUP_CHANNELS=1` on both MeshCore-enabled environments, plus `HasMeshCoreChat` now logs join/send success or failure explicitly instead of failing silently.
2. **`densaugeo/base64`'s `base64.hpp` defines its functions without `inline`.** Once `MAX_GROUP_CHANNELS` was defined (fix #1), MeshCore's own `BaseChatMesh.cpp` started including `base64.hpp` too (its real `addChannel()` body needs `encode_base64`/`decode_base64`) — and `HasMeshCoreChat.cpp` already included the same header directly, so both translation units now defined the same non-inline functions, a One Definition Rule violation the linker correctly rejected (`multiple definition of 'encode_base64_length'...`). Fixed by not including `base64.hpp` from `HasMeshCoreChat.cpp` at all — just forward-declaring the one function actually used there (`encode_base64`) and letting the linker resolve it against `BaseChatMesh.cpp`'s copy.

**Verification, over the air:** with both fixed, a Heltec V2 and the Heltec V3 — two independent physical nodes, no shared network, communicating purely over LoRa — successfully exchanged a real MeshCore channel message. V2's Serial log: `MC TX (sent): NOCALL-99>BEACON-99|QRZ`; V3's Serial log, moments later: `MC RX:HASviolet: NOCALL-99>BEACON-99|QRZ`. This is the first real proof the MeshCore integration's radio/crypto/routing path — not just its init sequence — actually works.

### MeshCore support for a second board (Heltec V2)

Extended past Heltec V3 to prove the packaging fixes above (ed25519 shim, GPIO shadowing workaround, LDF-discovery guards, transitive-dependency additions) were genuinely board-agnostic rather than V3-specific workarounds. They were: `env:heltec_wifi_lora_32_V2` needed only the board's own MeshCore variant config (`MC_VARIANT=heltec_v2`, `RADIO_CLASS=CustomSX1276`/`WRAPPER_CLASS=CustomSX1276Wrapper`, pins matching this board's existing native `LORA_*` pins exactly, confirmed independently from the same `pins_arduino.h` data used for native support) plus the two board-specific `-I` flags — every other fix (the shared `HasMeshCoreChat.h`/`.cpp`, `lib/MeshCoreEd25519Compat/`) applied unchanged and the build succeeded on the first attempt. `main.cpp` needed zero changes — board selection remains 100% build-flag-driven, same principle as every other board in this project.
...
8. ~~Confirm MeshCore mode on the real Heltec V3 (identity/channel join via Serial log), then an actual over-the-air channel message with a second MeshCore node.~~ **Done** — see "Two real bugs found getting an actual over-the-air message working" above. A Heltec V2 and the Heltec V3 exchanged a real message purely over LoRa.
9. ~~Extend MeshCore support to boards beyond Heltec V3~~ **Started** — Heltec V2 (SX1276) is done too, see above, and confirmed the packaging fixes are board-agnostic. Remaining: this project's other SX126x boards likely have official MeshCore variants too (`lilygo_t3s3`, `station_g2`, `lilygo_tbeam_supreme_SX1262` were all seen in MeshCore's own `variants/` listing during this session), unconfirmed against exact pin sets. The SX127x boards beyond Heltec V1/V2 (TTGO/T-Beam) would need their own MeshCore variant lookup too — not checked yet.
...
11. Consider whether `HasMeshCoreChat`'s `Serial.print` diagnostics (channel join, TX send success/failure) added while debugging item 8 are worth keeping long-term as basic operational visibility, or trimming back once MeshCore mode is otherwise trusted.
